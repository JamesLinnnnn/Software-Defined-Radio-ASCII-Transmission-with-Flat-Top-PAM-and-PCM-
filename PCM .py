...
print(f"Original binary signal from '{sd}':")
print(st)

# Print PCM result (flattened)
print("\nPCM Bitstream (flattened):")
print(dd)

# Group PCM output by sample (8 bits each)
print("\nPCM Bitstream (grouped by sample):")
grouped = np.reshape(dd, (-1, 8))  # 8 bits per sample
for i, sample in enumerate(grouped):
    print(f"Sample {i}: {''.join(map(str, sample.astype(int)))}")
